...
namespace Yazılım_Test_Aracı
{
    public partial class frmListOfProcess : Form
    {
        ...
        {
            frmInsertProcess ip = new frmInsertProcess(id, projectName, UsersID);
            ip.Show();
        }
        ...
        private void frmListOfProcess_Load(object sender, EventArgs e)
        {
            ...
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM tblProcess WHERE ParentID = 1 AND ProjectID = @ProjectID", con);
        ...
        private void dtGridProcess_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1 && e.ColumnIndex >= 0)
            {
                dtGridProcess.Rows[e.RowIndex].Selected = true;
            }
            
            int id = Convert.ToInt32(dtGridProcess.Rows[e.RowIndex].Cells[0].Value);
            string projectname = dtGridProcess.Rows[e.RowIndex].Cells[1].Value.ToString();
            if (dtGridProcess.Rows[e.RowIndex].Selected)
            {
                frmListOfSubProcess lomp = new frmListOfSubProcess(id, projectname, UsersID);
                lomp.Show();
            }
        }
    }
